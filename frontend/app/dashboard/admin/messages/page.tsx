  ...
  return (
    <div className="space-y-8">
      <div className="flex justify-between items-center bg-blue-100 p-6 rounded-2xl shadow-md">
        <h1 className="text-2xl font-bold text-blue-800">مدیریت پیام‌ها</h1>
        <p className="text-sm text-gray-600">مشاهده و پاسخ به تمام مکالمات</p>
      </div>
                ...
                >
                  <div className="flex justify-between items-center mb-1">
                    <span className="font-semibold text-gray-800">
                      {participants.map(p => p.full_name).join(", ") || "مکالمه"}
                    </span>
                    {lastMessage && (
          ...
          <section className="flex-1 flex flex-col">
            ...
            <header className="border-b border-gray-200 p-4 bg-white shadow-sm">
              <h2 className="font-semibold text-gray-800">
                {activeConversation.participants?.map(p => p.full_name).join(", ") || "مکالمه"}
              </h2>
              <p className="text-xs text-gray-500">
                {activeConversation.participants?.length || 0} شرکت‌کننده
              </p>
            </header>
